...
void joy_sub_cb(const sensor_msgs::Joy::ConstPtr &msg)
{
  ...
  if ( flag_mandatory_stoped_ && flag_planner_stop_cmds_ )
  {
    ...
    mars_quadrotor_msgs::PositionCommand cmd_msg;
...
void position_cmd_sub_cb(const mars_quadrotor_msgs::PositionCommand::ConstPtr &msg)
{
...
int main(int argc, char **argv)
{
  ...
  cmd_pub_ = nh.advertise<mars_quadrotor_msgs::PositionCommand>("/position_cmd", 10);
  ros::Subscriber joy_sub = nh.subscribe<sensor_msgs::Joy>("/joystick_from_bridge", 10, joy_sub_cb);
  ros::Subscriber cmd_sub = nh.subscribe<mars_quadrotor_msgs::PositionCommand>("/position_cmd", 100, position_cmd_sub_cb);
